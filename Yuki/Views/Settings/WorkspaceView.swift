...
struct MonitorElement: View {
        ...
        VStack(alignment: .center, spacing: 8) {
            ...
            ZStack {
                ...
                ZStack {
                    ...
                    ForEach(0..<min(3, monitor.workspaces.flatMap { $0.getAllWindowNodes() }.count), id: \.self) { index in
            ...
            .shadow(color: .black.opacity(isHovering ? 0.3 : 0.15),
                    ...
                    x: 0,
                    y: isHovering ? 5 : 2)
            .animation(.interactiveSpring(response: 0.25, dampingFraction: 0.7), value: isHovering)
    ...
    private func loadWallpaperAsync() {
        ...
        Task {
            ...
            if let screen = await NSScreen.screens.first(where: { NSEqualRects($0.frame, monitor.frame) }),
               let wallpaperURL = await NSWorkspace.shared.desktopImageURL(for: screen) {
