...
class ContainerNode: Node {
    ...
    private func layoutHorizontally(
        _ windows: [WindowNode], in frame: NSRect, force: Bool
    ) {
        let count = windows.count
        let totalWidth = frame.width
        let itemWidth =
            distributeEvenly
            ? (totalWidth - spacing * CGFloat(count - 1)) / CGFloat(count) : 0
        ...
        for (index, window) in windows.enumerated() {
            ...
            if force {
                window.move(
                    to: NSPoint(x: windowFrame.minX, y: windowFrame.minY))
                window.resize(
                    to: CGSize(
    ...
    private func layoutVertically(
        ...
        let itemHeight =
            distributeEvenly
            ? (totalHeight - spacing * CGFloat(count - 1)) / CGFloat(count) : 0

        for (index, window) in windows.enumerated() {
            let y =
                frame.maxY - CGFloat(index + 1) * (itemHeight + spacing)
                + spacing
            let windowFrame = NSRect(
                x: frame.minX,
                y: y,
