...
extension Notification.Name {
    ...
    static let windowUnminimized = Notification.Name(
        "com.yuki.WindowUnminimized")
    static let windowClosed = Notification.Name("com.yuki.WindowClosed")
    static let workspaceActivated = Notification.Name(
        "com.yuki.WorkspaceActivated")
    static let tilingModeChanged = Notification.Name(
        "com.yuki.TilingModeChanged")
}
...
class WindowObserverService {
    ...
    private func registerForSystemNotifications() {
        ...
        nc.addObserver(
            self, selector: #selector(handleAppHidden(_:)),
            name: NSWorkspace.didHideApplicationNotification, object: nil)
        nc.addObserver(
            self, selector: #selector(handleAppUnhidden(_:)),
            name: NSWorkspace.didUnhideApplicationNotification, object: nil)
        nc.addObserver(
            self, selector: #selector(handleAppDeactivated(_:)),
            name: NSWorkspace.didDeactivateApplicationNotification, object: nil)
        nc.addObserver(
            self, selector: #selector(handleAppTerminated(_:)),
            name: NSWorkspace.didTerminateApplicationNotification, object: nil)

        // Space changes
        nc.addObserver(
            self, selector: #selector(handleSpaceChanged(_:)),
    ...
    private func registerExistingApplications() {
        for runningApp in NSWorkspace.shared.runningApplications {
            if runningApp.activationPolicy == .regular {
                registerForWindowCreationNotifications(
    ...
    private func startWindowMoveObservation() {
        ...
        self.observationTimer = Timer.scheduledTimer(
            timeInterval: self.observationInterval,
            target: self,
            selector: #selector(self.checkForWindowMovements),
            userInfo: nil,
            repeats: true
        )
    }
    ...
    private func updateWindowPositionCache() {
        ...
        let windows = WindowManager.shared.windowDiscovery
    ...
    private func registerForWindowCreationNotifications(for pid: pid_t) {
        ...
        guard error == .success, let observer = observer else {
            print(
                "Failed to create AX observer for application with PID \(pid): \(error)"
            )
            return
        }
        ...
        AXObserverAddNotification(
            observer, appElement, kAXWindowCreatedNotification as CFString, nil)
        AXObserverAddNotification(
            observer, appElement,
            kAXFocusedWindowChangedNotification as CFString, nil)
        AXObserverAddNotification(
            observer, appElement,
            kAXApplicationActivatedNotification as CFString, nil)
        AXObserverAddNotification(
            observer, appElement, kAXWindowMiniaturizedNotification as CFString,
            nil)
        AXObserverAddNotification(
            observer, appElement,
            kAXWindowDeminiaturizedNotification as CFString, nil)
    ...
    @objc private func handleAppLaunched(_ notification: Notification) {
        if (notification.userInfo?[NSWorkspace.applicationUserInfoKey]
            as? NSRunningApplication)?.bundleIdentifier == lockScreenAppBundleId
        {
            return
        }

        // For application launch, register for window creation notifications
        if let launchedApp = notification.userInfo?[
            NSWorkspace.applicationUserInfoKey] as? NSRunningApplication
        {
            registerForWindowCreationNotifications(
                for: launchedApp.processIdentifier)
        }
    ...
    @objc private func handleAppActivated(_ notification: Notification) {
        ...
        if let activatedApp = notification.userInfo?[
            NSWorkspace.applicationUserInfoKey] as? NSRunningApplication
        {
    ...
    @objc private func handleAppHidden(_ notification: Notification) {
        if let hiddenApp = notification.userInfo?[
            NSWorkspace.applicationUserInfoKey] as? NSRunningApplication
        {
            WindowManager.shared.removeWindowsForApp(
                hiddenApp.processIdentifier)
        }
    }
    ...
    @objc private func handleAppTerminated(_ notification: Notification) {
        if let terminatedApp = notification.userInfo?[
            NSWorkspace.applicationUserInfoKey] as? NSRunningApplication
        {
    ...
    @objc private func checkForWindowMovements() {
        ...
        for windowInfo in windows {
            guard let windowId = windowInfo["kCGWindowNumber"] as? Int,
                ...
                let width = bounds["Width"] as? CGFloat,
                let height = bounds["Height"] as? CGFloat,
                let previousFrame = previousWindowPositions[windowId]
            else {
                continue
            }
            ...
            let positionChanged =
                abs(currentFrame.origin.x - previousFrame.origin.x)
                > positionThreshold
                || abs(currentFrame.origin.y - previousFrame.origin.y)
                    > positionThreshold

            // Check if size changed
            let sizeThreshold: CGFloat = 2.0
            let sizeChanged =
                abs(currentFrame.size.width - previousFrame.size.width)
                > sizeThreshold
                || abs(currentFrame.size.height - previousFrame.size.height)
                    > sizeThreshold
            ...
            let wasMinimized =
                previousFrame.width < 10 || previousFrame.height < 10
        ...
        let currentWindowIds = Set(
            windows.compactMap { $0["kCGWindowNumber"] as? Int })
        ...
        for windowId in newWindowIds {
            if let windowInfo = windows.first(where: {
                ($0["kCGWindowNumber"] as? Int) == windowId
            }),
                let bounds = windowInfo["kCGWindowBounds"] as? [String: Any],
                let x = bounds["X"] as? CGFloat,
                let y = bounds["Y"] as? CGFloat,
                ...
                let height = bounds["Height"] as? CGFloat
            {
                ...
                previousWindowPositions[windowId] = NSRect(
    ...
    func refreshWindowsOnMonitor(_ monitor: Monitor) {
        DispatchQueue.main.async {
            ...
            let visibleWindows = WindowManager.shared.windowDiscovery
                .getAllVisibleWindows()
            ...
            if let activeWorkspace = monitor.activeWorkspace {
                for windowInfo in visibleWindows {
                    ...
                    if monitor.contains(point: windowPosition) {
                        ...
                        if let window = WindowManager.shared.windowDiscovery
                            .getWindowElement(for: CGWindowID(windowId))
                        {
                            ...
                            // Register ownership
                            WindowManager.shared.windowOwnership[windowId] =
                                activeWorkspace.id
    ...
    func routeWindowEvent(_ eventType: WindowEventType, windowId: Int) {
        ...
        guard let workspaceId = WindowManager.shared.windowOwnership[windowId],
            let affectedMonitor = WindowManager.shared.monitors.first(where: {
                monitor in
                monitor.workspaces.contains(where: { $0.id == workspaceId })
            }),
            let workspace = affectedMonitor.workspaces.first(where: {
                $0.id == workspaceId
            })
        else {
            return
        ...
        if !workspace.isActive && eventType != .minimized
            && eventType != .unminimized && eventType != .closed
        {
            return
        }
...
private let axNotificationCallback: AXObserverCallback = {
    ...
    DispatchQueue.main.async {
        ...
        case kAXWindowCreatedNotification:
            ...
            let windows = WindowManager.shared.windowDiscovery
                .getWindowsForApplication(pid: pid)
            for window in windows {
                    ...
                    WindowManager.shared.windowOwnership[intId] == nil
                {
                    ...
                    if let activeWorkspace = WindowObserverService.shared
                        .getActiveWorkspaceForApp(pid)
                    {
                        activeWorkspace.adoptWindow(window)
            ...
            // Only refresh the active monitor
            if let activeMonitor = WindowManager.shared.monitorWithMouse {
                WindowObserverService.shared.refreshWindowsOnMonitor(
                    activeMonitor)
            }
        ...
        case kAXWindowMiniaturizedNotification:
                ...
                let intId = Int(exactly: windowId)
            {
                ...
                WindowObserverService.shared.routeWindowEvent(
                    .minimized, windowId: intId)
            }
