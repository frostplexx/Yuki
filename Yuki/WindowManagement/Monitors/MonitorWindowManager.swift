...
extension Monitor {
    ...
    func hideWorkspaceWindows(_ workspace: Workspace) {
        for windowNode in workspace.root.getAllWindowNodes() {
            ...
            if let systemWindowID = windowNode.systemWindowID {
                ...
                if WindowManagementState.originalWindowData[systemWindowID] == nil {
                    storeWindowData(windowNode: windowNode, windowId: systemWindowID)
                }
    ...
    func showWorkspaceWindows(_ workspace: Workspace) {
        ...
        for windowNode in windowNodes {
            if let systemWindowID = windowNode.systemWindowID,
               let savedData = WindowManagementState.originalWindowData[systemWindowID] {
                
                print("Restoring window \(systemWindowID) to position: \(savedData.position), size: \(savedData.size.width)x\(savedData.size.height)")
                ...
                WindowManagementState.originalWindowData.removeValue(forKey: systemWindowID)
            } else {
...
//            applyTilingLayout(workspace)
    ...
    func assignWindow(window: AXUIElement, windowId: Int, title: String? = nil) -> WindowNode? {
    ...
    private func storeWindowData(windowNode: WindowNode, windowId: Int) {
        guard let position = windowNode.position,
              let size = windowNode.size else { return }
        
        // Store both position and size together in a single structure
        WindowManagementState.originalWindowData[windowId] = WindowPositionData(position: position, size: size)
        
        print("Stored window \(windowId) position: \(position), size: \(size.width)x\(size.height)")
    }
...
//        guard let windowSize = windowNode.size else { return }
    ...
    private func positionWindowInDefaultLocation(_ windowNode: WindowNode) {
        ...
        let defaultSize = NSSize(width: visibleFrame.width * 0.6, height: visibleFrame.height * 0.6)
    ...
    private func disableAnimations<T>(for windowNode: WindowNode, _ operation: () -> T) -> T {
